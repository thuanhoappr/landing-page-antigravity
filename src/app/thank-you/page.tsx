...
import ThankYouCamNang from "@/components/ThankYouCamNang";
...
function ThankYouDefault() {
  return (
    ...
    >
      ...
      <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">Đăng Ký Thành Công!</h1>

      <p className="text-slate-400 text-lg mb-8">
        Cảm ơn bạn đã quan tâm đến Khóa huấn luyện Pickleball Thực chiến. Trợ lý Coach PPR Level 2 sẽ liên hệ lại với
        bạn trong vòng 24h tới để tư vấn lộ trình.
      </p>

      <div className="bg-slate-800/50 rounded-2xl p-6 mb-8 text-left border border-slate-700/50">
        <h3 className="text-white font-medium mb-2 flex items-center gap-2">Kết nối trực tiếp Coach:</h3>
...
function ThankYouContent() {
  ...
  return (
    <main className="min-h-screen bg-darker flex items-center justify-center p-4">
      ...
      {isCamNang ? <ThankYouCamNang invoice={invoice} /> : <ThankYouDefault />}
    </main>
  );
}
...
export default function ThankYouPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-darker" />}>
      <ThankYouContent />
    </Suspense>
  );
}
